...
5. 调用已有离线部署器，生成设备身份，配置 HTTP 接入、OpenAria 管理账号、服务与开机启动，等待 Device API 健康检查通过。
6. 安装 `openaria-update` 命令，并报告 HTTP 访问地址。

浏览器访问 `http://设备IP:8080/`，与现有局域网设备一致，无需证书确认或访问令牌。设备 Web/API 统一使用 HTTP，mDNS 也发布 HTTP 地址。新安装、升级及回退恢复旧配置时均使用该策略；旧证书和令牌文件保留在磁盘上但不再使用。OpenAria SSH 管理账号和救援热点使用项目现有默认策略，见 [README](../README.md#设备接入)。初次联网可使用有线网络或 RDK 系统现有网络。

首次通过 Wi-Fi 安装时，脚本会让 NetworkManager 保存当前连接的受管副本，并登记为启动后的恢复目标。系统识别仍在使用的原始连接，保持 Wi-Fi 在线；重启需要恢复连接时直接尝试 Wi-Fi，失败后才启用救援热点。原始连接配置保留，密码只在 NetworkManager 的受限配置中保存。热点配置关闭自动连接，避免它在启动时抢占 Wi-Fi。支持开放 Wi-Fi、系统已保存凭据的 WPA2/WPA3 个人网络；企业认证或依赖桌面密码代理的连接会在激活前停止，需要通过有线网络安装。已有用户网络选择不自动覆盖；只有未经用户配置的首次启动热点默认状态允许继承当前已连接的 Wi-Fi。升级前仍应保持其他客户端空闲。
...
下载入口使用 `umask 077` 保护临时文件，调用固件安装器时单独使用 `022`，确保服务账号能够读取和执行新建发行目录。凭据权限仍由部署器显式设置。如果使用过 2026-09-10 修复前的入口，并在首次安装时遇到数据卷服务 `Permission denied`，应先保留失败日志并核对 `/opt/rp-ylx` 的逐级权限；普通重试不会自动重建已经存在的失败发行目录。不要对配置、凭据或数据卷执行递归放宽权限。
