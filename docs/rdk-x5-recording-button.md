...
默认使用 `/sys/class/leds/ACT` 对应的板载状态灯。RDK X5 官方硬件文档将其标为
[橙色状态指示灯](https://developer.d-robotics.cc/rdk_doc/Quick_start/hardware_introduction/rdk_x5/)。
启用此功能后，它由原先的系统心跳闪烁改为以下含义：

| 灯光 | 含义 |
| --- | --- |
| 熄灭 | 空闲，且没有待处理的录制失败 |
| 常亮 | 正在录制 |
| 慢闪，每秒一轮 | 正在停止、保存录制；等灯熄灭后再断电 |
| 快闪，每秒四轮 | 录制异常、设备被阻塞或暂时无法读取录制状态 |

灯跟随 Device API 的实际状态，每 0.5 秒检查一次，因此用实体按钮或网页开始、停止
都能同步更新。按键请求失败不会直接把灯改成“录制中”。灯控制在独立线程中运行，
不会阻塞按钮采样；LED 缺失或不可写时只记录日志，按钮仍可正常工作。

现有按钮配置不用增加字段即可启用 `ACT`。如需单独关闭录制灯，在
`/etc/rp-ylx/recording-button.json` 中加入 `"status_led": null` 并重启按钮服务。
