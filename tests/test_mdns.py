...
class MdnsPublisherTest(unittest.TestCase):
    def test_runtime_addresses_prefer_the_authoritative_default_route(self) -> None:
        runtime = {
            "network": {
                "ap": {"addresses": ["10.42.0.1/24", "invalid"]},
                "wifi_client": {"addresses": ["198.51.100.36/24", "127.0.0.1/8"]},
                "wired": {"addresses": ["203.0.113.10/24", "169.254.1.2/16"]},
                "default_route": "wifi_client",
            }
        }
        with patch("rp_ylx.mdns.collect_linux_runtime", return_value=runtime):
            self.assertEqual(runtime_ipv4_addresses(), ("198.51.100.36",))

    def test_runtime_addresses_fall_back_to_the_rescue_ap_without_a_default_route(self) -> None:
        runtime = {
            "network": {
                "ap": {"addresses": ["10.42.0.1/24"]},
                "wifi_client": {"addresses": []},
                "wired": {"addresses": ["203.0.113.10/24"]},
                "default_route": "none",
            }
        }
        with patch("rp_ylx.mdns.collect_linux_runtime", return_value=runtime):
            self.assertEqual(runtime_ipv4_addresses(), ("10.42.0.1",))
