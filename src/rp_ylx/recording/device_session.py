...
class DeviceSessionRecorder:
    ...
    def _segment_record(self, segment: ClosedSegment) -> dict[str, object]:
        ...
        for eye, relative, declared in (
            ...
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_size != declared:
                raise DeviceRecordingError(
                    "segment_invalid", f"分段 {relative} 的大小与助手回报不一致"
                )
            digest = _digest(path)
            metadata = path.stat(follow_symlinks=False)
            if metadata.st_size != declared:
                raise DeviceRecordingError("segment_invalid", f"分段 {relative} 在校验期间发生变化")
            self._artifact_identities[relative] = (
                metadata.st_dev,
                metadata.st_ino,
                metadata.st_size,
    ...
    def _finish_audio(self) -> None:
        ...
        for expected_index, raw_segment in enumerate(segments):
            ...
            metadata = path.stat(follow_symlinks=False)
            if not stat.S_ISREG(metadata.st_mode):
                raise DeviceRecordingError("audio_invalid", "音频 artifact 不是普通文件")
            digest = _digest(path)
            metadata = path.stat(follow_symlinks=False)
            identity = (
                metadata.st_dev,
            ...
            self._artifact_identities[relative] = identity
            self._audio_bytes += metadata.st_size
